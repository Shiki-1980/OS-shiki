...
void syscall_entry(UserContext *context)
{
    ...
    u64 id = context->x[8];  // 从 x8 获取系统调用 ID
    if (id >= NR_SYSCALL) {
        PANIC();  // 如果 ID 超出范围，则触发 Panic
    }

    // 提取参数，分别存储在 x0 到 x5 中
    ...
    if (syscall_table[id] != NULL) {
        // 调用系统调用并将返回值保存到 x0
        context->x[0] = ((u64 (*)(u64, u64, u64, u64, u64,u64))syscall_table[id])(arg0, arg1, arg2, arg3, arg4, arg5);
    } else {
        PANIC();  // 如果系统调用函数不存在，则触发 Panic
    }
}
/** 
